...
for elty in (Float32,Float64)
    @eval begin
        ...
        function A_mul_Bt!(C::StridedMatrix{Complex{$elty}}, A::StridedMatrix{$elty}, B::StridedMatrix{Complex{$elty}})
            Bfl = reinterpret($elty,B,(2size(B,1),size(B,2)))
            Cfl = reinterpret($elty,C,(2size(C,1),size(C,2)))
            gemm_wrapper!(Cfl,'N','T',Bfl,A)
            return C.'
        end
    end
end
...
for elty in (Float32,Float64)
    @eval begin
        function At_mul_Bt!(C::StridedMatrix{Complex{$elty}}, A::StridedMatrix{$elty}, B::StridedMatrix{Complex{$elty}})
            Bfl = reinterpret($elty,B,(2size(B,1),size(B,2)))
            Cfl = reinterpret($elty,C,(2size(C,1),size(C,2)))
            gemm_wrapper!(Cfl,'N','N',Bfl,A)
            return C.'
        end
    end
end
